...
/**
 * FillRangeCommand - Handles fill handle operations with undo/redo support
 * Delegates to UpdateCellsCommand pattern for execution
 */
export class FillRangeCommand extends Command {
  /**
   * @param {Object} params
  ...
  constructor({ cellUpdates, fileManager, formulaWorker, renderer }) {
    ...
    if (!cellUpdates || cellUpdates.length === 0) {
      throw new Error('FillRangeCommand requires at least one cell update');
    }

    if (!fileManager) {
      throw new Error('FillRangeCommand requires fileManager');
    }

    if (!formulaWorker) {
      throw new Error('FillRangeCommand requires formulaWorker');
    }
  ...
  /**
   * Applies updates (same pattern as UpdateCellsCommand)
   * @private
   */
