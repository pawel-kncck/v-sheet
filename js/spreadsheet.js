class Spreadsheet {
  ...
  _initEventListeners() {
    ...
    // --- UPDATED: Header listeners now handle both selection and resize ---
    ...
    window.addEventListener('mouseup', () => {
      this.isMouseDown = false;
      // Stop resizing if it was in progress
      if (this.isResizing) {
        this._stopResize();
      }
    });
    ...
    // Double-click to edit (unchanged)
  ...
  _getSelectedHeaderIndices(type, clickedIndex) {
    ...
    const isFullCol = (sel) => {
      ...
      // A "column selection" must span all rows
    ...
    const isFullRow = (sel) => {
      ...
      // A "row selection" must span all columns
      return minCol === 0 && maxCol === this.COLS - 1;
    };

    let inSelection = false;

    // Check if the clicked header is part of a *relevant header selection*
    for (const sel of this.selections) {
      // If we are resizing a column, only check full-column selections
      ...
      // If we are resizing a row, only check full-row selections
    ...
    // If it is, resize all *relevant* selected columns/rows
    if (inSelection) {
      this.selections.forEach((sel) => {
        // Again, only loop over relevant selection types
        if (checkCol && isFullCol(sel)) {
          const min = Math.min(sel.start.col, sel.end.col);
          const max = Math.max(sel.start.col, sel.end.col);
  ...
  _onResize(e) {
    ...
    // Destructure the new clickedIndex property
    const {
      type,
      startPos,
      indices,
    ...
    // Get the original size of the *clicked* header
    const clickedOriginalSize = originalSizes[clickedIndex];

    // Calculate the new size based *only* on the clicked header
    let newSize;
    if (type === 'col') {
    ...
    // Apply that *same new size* to all selected indices
  ...
  _stopResize() {
    ...
    // Mouseup is already removed with { once: true }
  }
